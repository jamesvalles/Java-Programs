...
public class HW3Driver {
	...
	public static void testHeight2() {
		testHeight("C",0);
		testHeight("CA",1);
		testHeight("CAD",1);		 // complete BT with 3 nodes
		testHeight("HCADMLZ",2);  // complete BT with 7 nodes
		testHeight("HCADMJZGL",3);// complete BT(7) w/ two extra nodes
	}
	/* from
	...
	public static void testHeight( String keys, int answer ) {

		// create and populate the table from the input string 
		BST403<String,String> aTree = from(keys,keys);
		//  do the test
		int result = aTree.heightTree();

		if ( result == answer)  // test passes
			StdOut.format("Height Tree: Correct   Keys: [ %s ]   Answer: %d\n", keys, answer);
		else
			StdOut.format("Height Tree: *Error*   Keys: [ %s ]   Answer: %d\n", keys, answer);
	}
